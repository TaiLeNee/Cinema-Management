...
// Cấu trúc để lưu thông tin ghế
struct Chair {
    int id;                 // ID của ghế
    wstring name;           // Tên ghế
    bool isBooked;         // Trạng thái ghế có bị ẩn không

    Chair(int id, wstring name, bool isHidden) : id(id), name(name), isBooked(isHidden) {}
};

// Kiểm tra xem ghế có bị ẩn không
bool isHidden(const Chair& chair) {
    return chair.isBooked;
}
...
void drawSeatRow(char rowLabel, const vector<Chair>& chairs, int rowIndex, int numSeats) {
    ...
    for (int j = 0; j < numSeats; ++j) {
        ...
        if (isHidden(chair)) {
            ...
            wcout << L"| " << background << L" " << setw(3) << chair.name <<  L" " << RESET << L" ";
        }
    }
    wcout << L"|" << endl;
}
...
void drawLegend() {
    ...
    wcout << endl << L"Chú thích:" << endl;
    wcout << WHITE_BG << L"       " << RESET << L" - Ghế thường" << endl;
    wcout << PINK_BG << L"       " << RESET << L" - Ghế VIP" << endl;
